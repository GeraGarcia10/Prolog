...
inicio :-
    ...
    send_list(Dialog, append,
              [ button('Plantas Medicinales', message(@prolog, mostrar_plantas)),
                ...
                button('Informacion que cura', message(@prolog, seleccion_planta)),  % Nuevo botón
                button('Palabras raras', message(@prolog, palabras_raras)),  % Nuevo botón
...
mostrar_planta(Planta) :-
    ...
    new(Dialog, dialog('* Informacion de Planta')),
...
%seccion glosario
palabras_raras :-
    new(Dialog, dialog('Glosario')),
    send(Dialog, size, size(560, 750)),  % Establecer el tamaño del diálogo
    send(Dialog, display, new(BG, bitmap('C:/Users/Infectedblood/Desktop/Yerberito/Imagenes/Raras.jpg'))),  % Cargar la imagen
    send(BG, size, Dialog?size),  % Establecer el tamaño de la imagen igual al tamaño del diálogo
    send(BG, position, point(0, 0)),  % Establecer la posición de la imagen en la esquina superior izquierda
    send_list(Dialog, append,
              [ button('Regresar', message(Dialog, destroy))  % Botón para cerrar la ventana
              ]),
    send(Dialog, open).    

% Selección de planta
seleccion_planta :-
    new(Dialog, dialog('Seleccion de Planta')),
    send(Dialog, size, size(1080, 700)),  % Establecer el tamaño del diálogo
    send(Dialog, display, new(BG, bitmap('C:/Users/Infectedblood/Desktop/Yerberito/Imagenes/Seleccion.jpg'))),
    send(BG, size, Dialog?size),
    send_list(Dialog, append,
              [ new(Planta, menu('Seleccione una planta', cycle)),
                button('Continuar', message(@prolog, mostrar_imagen_planta, Planta?selection)),
                button('Regresar', message(Dialog, destroy))
              ]),
    % Agregar las plantas al menú desplegable
    findall(NombrePlanta, planta(NombrePlanta), Plantas),
    send_list(Planta, append, Plantas),
    send(Dialog, open).

% Mostrar imagen de la planta seleccionada
mostrar_imagen_planta(PlantaSeleccionada) :-
    new(Dialog, dialog('Informacion de la Planta')),
    atom_concat('C:/Users/Infectedblood/Desktop/Yerberito/Imagenes/', PlantaSeleccionada, ImagenPath1),
    atom_concat(ImagenPath1, '.jpg', ImagenPath),
    new(Imagen, bitmap(ImagenPath)),
    send(Dialog, display, Imagen, point(50, 50)),  % Ajustar la posición de la imagen
    send(Dialog, append, button('Regresar', message(Dialog, destroy))),
    send(Dialog, open).
